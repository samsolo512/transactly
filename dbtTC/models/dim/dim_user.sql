...
with
    ...
    ,final_logic as(
        select
            ...
            ,ul.lead_flag  --**********
            ,case
                when ul.lead_flag = 0 and tc_client_flag = 1 then 'TC client only'
                when ul.lead_flag = 1 and tc_client_flag = 0 then 'SF lead only'
                when ul.lead_flag = 1 and tc_client_flag = 1 then 'TC client and SF lead'
                when ul.lead_flag = 0 and tc_client_flag = 0 then 'TC client only'
                when ul.lead_flag = 0 and tc_client_flag = 0 and u.user_id is not null and u.user_id <> 0 then 'TC client only'
...
-- select client_type, count(1) from final group by client_type order by count(1) desc
